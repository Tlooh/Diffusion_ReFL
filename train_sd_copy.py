...
def main():
    ...
    def tokenize_captions(examples, caption_name = "complex",is_train=True):
        ...
        if caption_name == "complex":
            caption_column = complex_caption_column
        else:
            caption_column = simple_caption_column
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                complex_encoder_hidden_states = text_encoder(batch["complex_input_ids"].to(accelerator.device))[0]
                print(batch["complex_input_ids"].shape)
                simple_encoder_hidden_states = text_encoder(batch["simple_input_ids"].to(accelerator.device))[0]
                ...
                # 设置噪声模型的时间步数，从而决定模型在每个时间步长上的噪声情况。
